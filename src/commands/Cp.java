...
public class Cp extends DirectoryManager implements CommandI {
	 ...
	 * 
	 * @param filesys  Filesystem to be mutated when copying and pasting file/directory
	 * @param args  Array containing arguments such as the old path to the new path information
	...
	/**
	 * Initializes the pathFrom, pathTo, newDir and filename variables, this is needed
	 * to traverse the system to copy and paste into the right places
	 * 
	 * @param fs  Filesystem to be mutated with new files
	 * @throws DirectoryException  throws an exception when invalid arguments or directory is given
	 */
	...
	/**
	 * Traverses to the first path given to copy the node. This node is then bundled into toMove to be
	 * pasted later
	 * 
	...
	/**
	 * Take the newly created toMove node and pastes it into the second path provided
	 * 
	 * @param fs  Filesystem to be mutated
	 * @return  A string for user if need be

	 * @throws DirectoryException  Throws a directory exception if second path is invalid
	 */
	...
	/**
	 * Checks if the second path has the folder needed to paste the node into, if not
	...
	public void checkDirectory(final FileSystemI fs){
		...
		//This runs if the folder wasn't found, this creates the new folder and adds it to the dir
		//above it. It then assigns this new folder as the current position of the user.
