  ...
  public String run(String[] args, String fullInput, boolean val) {   
    ...
    //If the user provided 1 argument  
      ...
      if(numeric && number >= 0 && number % 1 == 0) {
        ...
        output = error.getError("Invalid Argument", args[0] + " is not either a number or positive or an integer");
      }
    //If the user provided multiple arguments  
    }else if(args.length > 1) {
      
      //Collects the user input from the fullInput
      String parameter = fullInput.substring(fullInput.indexOf("history") + 7).trim();
      //Returns an error of Mulptile parameters provided
      output = error.getError("Mulptile parameters provided", parameter + " , either one or no input");
    }
