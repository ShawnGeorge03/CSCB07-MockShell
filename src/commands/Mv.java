...
public class Mv extends DirectoryManager implements CommandI {
	...
	String newDir;
	/**
	 * parentToMove is the node of the directory that the removed node needs to be added to
	 * toMove is the node to move
	 */
	Node parentToMove;
	Node toMove = null;
	/**
	...
	/**
	 * Run method for this class which will execute the mv functionality in the filesystem
	 * 
	 ...
	 * @throws DirectoryException  Throws a directory exception if a new directory to be created has
	...
	public void initPathandFile(FileSystemI fs) throws DirectoryException{
		...
		//Important for finding path to move node to, and possible new folder to create to add 
		//new node into
	 ...
	 * 
	 * @throws InvalidArgsProvidedException  Throws exception if arguments are invalid
	 ...
	 * @throws DirectoryException  Throws exception if pathFrom isn't a valid path or file wasn't found
	 */
	...
	/**
	 * Checks if new folder needs to be created, if so then create it
	 * if not then traverse into that folder
	 * 
	 * @param fs  Filesystem to be mutated
	 */
	public void checkDirectory(FileSystemI fs){
		//Checks if directory exists with the same name, if so, assign that as current and return
		for (int i = 0; i < parentToMove.getList().size(); i++) {
		...
		//If directory doesn't exist, create it, add it, and assign it as the current node
